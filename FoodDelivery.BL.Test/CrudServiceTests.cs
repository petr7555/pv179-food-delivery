...
public class CrudServiceTest
{
    ...
    private readonly List<TestEntity> _mockDb;
    ...
    [Fact]
    public async Task ItGetsByIdAsync()
    {
        var testEntity1 = await _service.GetByIdAsync(1);
        testEntity1.Should().BeEquivalentTo(_mapper.Map<TestDto>(_entity1));

        var testEntity2 = await _service.GetByIdAsync(2);
        testEntity2.Should().BeEquivalentTo(_mapper.Map<TestDto>(_entity2));
    }

    [Fact]
    public async Task ItGetsAllAsync()
    {
